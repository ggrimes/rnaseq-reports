...
if (grepl("hg|human", genome, ignore.case = TRUE)) {
  ens <- AnnotationHub::query(ah, c("Homo sapiens", "EnsDb"))
  ens <- ens[["AH119325"]]
} else if(grepl("mm|mouse", genome, ignore.case = TRUE)) {
  ens <- AnnotationHub::query(ah, c("Mus musculus", "EnsDb"))
  ens <- ens[["AH116340"]]
} else {
...
rdata <- ensembldb::genes(ens, return.type = "data.frame")  %>%
  dplyr::select(gene_id, gene_name, entrez=entrezid,
...
# rdata <- rdata %>% filter(!grepl("ncRNA", gene_biotype)) %>% 
#   filter(!grepl('Mir', gene_name)) %>% # other examples
#   filter(!grepl('Snor', gene_name))
# counts <- counts[rdata$gene_id,]
```
...
pca$plot + ggtitle(paste0("All samples", "\nPCA using ", nrow(vsd_before), " genes")) +
  ...
  geom_mark_ellipse(aes(color = .data[[column]])) +  scale_color_grafify("kelly")
```
