...
# imp <- cbind(as.numeric(as.factor(as.character(treatment1))),as.numeric(as.factor(as.character(treatment2))), as.numeric(as.factor(as.character(treatment3))))
```
...
This volcano plot shows the genes that are significantly up- and down-regulated as a result of the analysis comparison. The points highlighted in purple are genes that have padj < 0.05 and a log2-fold change > 0.5. Points in pink have a padj < 0.05 and a log2-fold change < 0.5 and points in blue have a padj > 0.05 and a log2-fold change > 0.5. Grey points are non-significant. The dashed lines correspond to the cutoff values of log2 foldchance and padj that we have chosen.
...
for (contrast in names(de_list)) {
  # Create a tabset of the contrast
  cat("### ", contrast, "\n\n")
  # Retrieve the full differential expression data frame for the contrast
  res <- de_list[[contrast]][["all"]]
  # Re-assign the full differential expression data frame for the contrast to `res_mod`
  res_mod <- res

  # NOTE: The below line of code will change anything with an absolute(lfc) above 5 to a 5. This will help remove outliers to better frame the graph if wanted.
  # res_mod <- res %>% mutate(lfc = replace(lfc, lfc < -5, -5)) %>% mutate(lfc = replace(lfc, lfc > 5, 5))

  # Extract the log-fold change, adjusted p-value and gene name for the top 10 genes with the highest value of pi (abs(lfc) * -log10(padj)) and aggin the dataframe to the object show
  show <- as.data.frame(res_mod[1:10, c("lfc", "padj", "gene_name")])
  # Create a Volcano plot
  p1 <- EnhancedVolcano(res_mod, # Dataframe to use
    lab = res_mod$gene_name, # All gene names
    pCutoff = 0.05, # P-value threshold
    selectLab = c(show$gene_name), # Select the gene names to highlight from the high pi value genes
    FCcutoff = 0.5, # Absolute fold-change cutoff to apply
    x = "lfc", # Column from the res_mod object to plot on the x-axis
    y = "padj", # Column from the res_mod object to plot on the y-axis
    title = contrast, # Add a title with the name of the contrast
    col = c("darkgrey", "lightblue", "plum1", "purple"), # Assign colors to use
    # xlim = c(-5,5),  # Set the x-axis to only show some points
    subtitle = "", # Provide a subtitle
    drawConnectors = TRUE, # Whether to draw lines from the labels to the points
    max.overlaps = Inf # How many labels to allow to overlap, `Inf` will show all labels
  )
...
for (contrast in names(de_list)) {
  ...
  # Extract the significant genes from variance stabilized transformation count data and assign them to the object ma
  ma <- norm_matrix[res_sig$gene_id, ]
  # Extract the factor of interest column from the metadata and assign it to the object colma
  colma <- coldata[, c(column), drop = FALSE]
  # Assign the colors to use for the annotation for each factor of interest to a list called ma_colors
  ma_colors <- lapply(colnames(colma), function(c) {
    # Assign colors to l.col based upon how many factor levels are in the factor of interest
    l.col <- colors[1:length(unique(colma[[c]]))]
    # Take the factor levels in the factor of interest, sort them based upon their underlying integer value, then assign them to the names of the l.col vector
    names(l.col) <- sort(unique(colma[[c]]))
    # Print the l.col object
    l.col
  })
  # Assign the column names of the factors of interest to the names of each object within the ma_colors list
  names(ma_colors) <- colnames(colma)
  # Create heatmap
  p1 <- pheatmap(ma, # Use variance stabilized transformation count data to create a heatmap
    color = inferno(10), # Select the color gradient inferno from the Viridis color pallette with 10 equally spaced colors
    cluster_rows = TRUE, # Whether the data should be clustered by rows
    show_rownames = FALSE, # Whether the row names should be displayed
    annotation = colma, # Data frame of factor of interest
    annotation_colors = ma_colors, # List of containing vectors to determine the coloring of the factor of interest
    border_color = NA, # Color of cell borders, NA will result in no border
    fontsize = 10, # Base font size for the heatmap
    scale = "row", # Direction that the data should be scaled by
    fontsize_row = 10, # Font size for the row names
    height = 20 # Height of the output figure in inches
  )
  print(p1) # Print the plot
  cat("\n\n") # Print two new lines
}
...
dt_list <- list()
# For each of the contrasts in the de_list list object to extract the name of the contrast and the differentially expressed genes to be rendered into an HTML table
for (contrast in names(de_list)) {
  # Retrieve the significantly differential expressed genes data frame for the contrast
  res_sig <- de_list[[contrast]][["sig"]]
...
# NOTE: fig-height may need to be adjusted to get the plots to show up better
# Number of genes to present
n <- 16

# For each of the contrasts in the de_list list object
for (contrast in names(de_list)) {
  # Create a tabset of the contrast
  cat("### ", contrast, "\n\n")
  # Retrieve the significantly differential expressed genes data frame for the contrast
...
for (contrast in names(de_list)) {
  ...
  res_sig <- fa_gsea_list[[contrast]] %>% # Use the GSEA obect for a given contrast. NOTE: These have already been filter for adjusted p-value less than 0.05.
    filter(padj < 0.05) # Filter the for adjusted p-values of less than 0.05
  # Populate the dt_list list
  dt_list <- c(
    dt_list, # With the contents of dt_list from the previous loop
    list(h3(contrast)), # Odd numbered objects in the list will be the contrast name in a HTML renderable Heading 3 size
...
```{r print-ora-all, results='asis', eval=run_FA}
# NOTE DT::datatables doesn't work with tabset and for loops
# You can use the following code to print dynamically or call manually sanitize_datatable() multiple times
...
for (contrast in names(de_list)) {
  ...
  dt_list <- c(
    dt_list, # With the contents of dt_list from the previous loop
    list(h3(contrast)), # Odd numbered objects in the list will be the contrast name in a HTML renderable Heading 3 size
    list(sanitize_datatable(res_sig)) # Even numbered object in the list will hold HTML renderable data table from the DT package containing the significantly over-represented terms for genes with a negative and positive lfc
  )
}

# Render the dt_list into HTML for the significantly over-represented terms for genes with a negative and positive lfc
tagList(dt_list)
```
...
saveRDS(coldata, "DEGpattern_deseq_meta.rds")
