...
opts_chunk[["set"]](
  ...
  echo = T, # Whether to include R source code in the final knitted document
...
# Note: vsd_before was previously assigned in the setup_RUV code chunk so this is overwriting that
vsd_before <- vst(dds_to_use)
# Extract the variance-stabilized transformed (normalized) counts and assign them to norm_matrix
# Note: norm_matrix was previously assigned in the setup_RUV code chunk so this is overwriting that
norm_matrix <- assay(vsd_before)
# We are going to modify coldata in the RUV section (thus why we are creating new_cdata), but we will also want to retain the original
new_cdata <- coldata
```
...
pca2 + scale_color_grafify("kelly")
```
...
pca_combat <- degPCA(norm_matrix, coldata,
  condition = column
) + ggtitle("After Combatseq")
pca_combat + scale_color_grafify("kelly")
```
...
de_list <- lapply(contrasts, function(contrast) {
  ...
  res_sig <- res %>%
    filter(padj < 0.05) %>%
    arrange(padj) %>%
    mutate(gene_name = ifelse(is.na(gene_name),
      gene_id, gene_name
    ))
  results <- list(
    lfc = resLFC, lfcs = resLFCS,
    all = res, sig = res_sig
  )
  return(results)
})
...
fa_gsea_list <- lapply(de_list, function(contrast) {
  ...
  input_entrezid <- AnnotationDbi::select(ann.org, gsea_input$gene_id, "ENSEMBL", columns = c("ENTREZID", "SYMBOL"))
  input_entrezid <- inner_join(gsea_input, input_entrezid, by = c("gene_id" = "ENSEMBL")) %>%
    filter(!is.na(ENTREZID)) %>%
    distinct(ENTREZID, .keep_all = TRUE) %>%
    arrange(desc(lfc))

  tb <- run_fgsea_v2(input_entrezid, all_in_life)
  tb %>% filter(padj < 0.05)
})
```
...
fa_list <- lapply(de_list, function(contrast) {
  ...
  universe <- res %>%
    filter(!is.na(padj)) %>%
    pull(gene_id)
  universe_mapping <- rdata %>% filter(gene_id %in% universe, !is.na(entrez))
  # AnnotationDbi::select(ann.org, universe, 'ENSEMBL', columns=c('ENTREZID', 'SYMBOL'))

  ora_input <- res %>%
    filter(!is.na(padj), padj < 0.01, abs(lfc) > 0.3) %>%
    pull(gene_id)
  # change to the right species
  ...
  ora_input <- res %>%
    filter(!is.na(padj), padj < 0.01, lfc > 0.3) %>%
    pull(gene_id)
  # change to the right species
  input_entrezid <- rdata %>% filter(gene_id %in% ora_input, !is.na(entrez))
  # AnnotationDbi::select(ann.org, ora_input, 'ENSEMBL', columns = c('ENTREZID', 'SYMBOL'))
  up <- run_fora_v2(input_entrezid, universe_mapping, all_in_life)
  ...
  input_entrezid <- rdata %>% filter(gene_id %in% ora_input, !is.na(entrez))
  # AnnotationDbi::select(ann.org, ora_input, 'ENSEMBL', columns = c('ENTREZID', 'SYMBOL'))
  down <- run_fora_v2(input_entrezid, universe_mapping, all_in_life)
...
for (contrast in names(contrasts)) {
  name_deg_fn <- file.path(
    basedir,
    str_interp("${filename}_${contrast}_deg.csv")
  )

  name_pathways_fn <- file.path(
    basedir,
    str_interp("${filename}_${contrast}_pathways.csv")
  )
