...
run_fora_v2 <- function(input, uni, all_in_life) {
  ...
  pathways_ora_all <- lapply(names(all_in_life), function(database) {
    ...
    bind_rows() %>%
    mutate(analysis = "ORA")
  ora_tb <- pathways_ora_all %>%
    unnest(overlapGenes) %>%
    group_by(pathway) %>%
    left_join(uni, by = c("overlapGenes" = "entrez")) %>%
    dplyr::select(
      pathway, padj, NES, gene_name, analysis,
      database
    ) %>%
...
run_fgsea_v2 <- function(input, all_in_life) {
  ...
  tb <- pathways_all %>%
    ...
    dplyr::select(
      pathway, padj, size, NES, SYMBOL, analysis,
      database
    ) %>%
...
run_fora <- function(input, uni, all_in_life) {
  ...
  ora_tb <- pathways_ora_all %>%
    ...
    dplyr::select(
      pathway, padj, NES, SYMBOL, analysis,
      database
    ) %>%
    group_by(pathway, padj, NES, database, analysis) %>%
    summarise(genes = paste(SYMBOL, collapse = ","))
